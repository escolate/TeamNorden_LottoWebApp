            ...
            case 'save':
                ...
                if ($winner->getWin_notificated() == '') {
                    ...
                    if ($winner->getUser()->getUse_email() != '' && $not->send()) {
                        ...
                        $log->setLog_action("Gewinnbenachrichtigung wurde an {$winner->getUser()->getUse_email()} verschickt.");
                        $log->send();
                        MysqlAdapter::getInstance()->saveLog($log);
                    } else {
                        $log = new Log();
                        $log->setLog_level(Log::WARNING);
                        $log->setLog_action("Gewinnbenachrichtigung an {$winner->getUser()->getUse_email()} ist fehlgeschlagen!.");
                        $log->send();
                        MysqlAdapter::getInstance()->saveLog($log);
                    }
                }
                header("Location: " . URI_EVENT . "/" . $winner->getSeries()->getEvent()->getEvt_id() . "-" . $winner->getSeries()->getEvent()->getEvt_name(), TRUE, 303);
                exit();
                break;
            case 'add':
                $ser_id = $_POST['ser_id'];
                ...
                break;
            case 'cancel':
                if (!is_array($_POST['win_id'])) {
                    $win_id = array($_POST['win_id']);
                } else {
                    $win_id = $_POST['win_id'];
                }

                foreach ($win_id as $id) {
                    $winner = MysqlAdapter::getInstance()->getWinner($id);
                    $winner->setWin_del('1');
                    MysqlAdapter::getInstance()->saveWinner($winner);
                }
