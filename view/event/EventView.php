...
<div class="content-box">
    ...
    <div class="list">
	<form action="/veranstaltung/create" method="post">
	    <table>
		<thead>
		    <tr>
		    <th></th>
...
HTML;
	...
	foreach ($this->vars['eventList'] as $object) {
	    ...
	    echo "<td><a href=\"/veranstaltung/{$object->getEvt_id()}-{$object->getEvt_name()}\"> {$this->vars['eventCreatorList'][$i]->getUse_firstname()} {$this->vars['eventCreatorList'][$i]->getUse_lastname()}</a></td>";
	    echo "<td><a href=\"/veranstaltung/{$object->getEvt_id()}-{$object->getEvt_name()}\">{$this->getDate($object->getEvt_cre_dat())} - {$this->getTime($object->getEvt_cre_dat())}</a></td>";
	    echo '</tr>';
	    $i++;
	}
